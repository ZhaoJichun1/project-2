...
from test_loss import loss_test
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                loss.backward()
                # 累计梯度
                if (i + 1) % 4 == 0 or (i + 1) == len(self.train_dataloader):
                    self.optimizer.step()
                    self.optimizer.zero_grad()
                ...
                if i%10==0:
                    ...
                    writer.add_scalar('\\Train/loss_giou\\', mloss[0], itr)
                    writer.flush()
                    writer.add_scalar('\\Train/loss_conf\\', mloss[1], itr)
                    writer.flush()
                    writer.add_scalar('\\Train/cls\\', mloss[2], itr)
                    writer.flush()
                    writer.add_scalar('\\Train/loss_total\\', mloss[3], itr)
                    writer.flush()
                    print(s)

                # multi-sclae training (320-608 pixels) every 10 batches
                if self.multi_scale_train and (i+1)%40 == 0:
            ...
            if (epoch+1) % 5 == 0 or epoch > 30:
                ...
                with torch.no_grad():
                    if (epoch+1) % 5 == 0:
                        loss_test(self.yolov3, epoch, self.device, writer)
                    APs = Evaluator(self.yolov3).APs_voc()
                    for i in APs:
                        print("{} --> mAP : {}".format(i, APs[i]))
                        mAP += APs[i]
                    mAP = mAP / self.train_dataset.num_classes
                    writer.add_scalar('\\test/mAP\\', mAP, epoch)
                    writer.flush()
                    print('mAP:%g'%(mAP))
