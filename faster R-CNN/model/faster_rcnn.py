...
class FasterRCNN(nn.Module):
    ...
    def predict_rpn(self, imgs, sizes=None):
        self.eval()
        prepared_imgs = imgs
        n_rpn = self.rpn.proposal_layer.n_test_post_nms
        self.rpn.proposal_layer.n_test_post_nms = 20
        for img, size in zip(prepared_imgs, sizes):
            img = at.totensor(img[None]).float()
            scale = img.shape[3] / size[1]
        ...
        self.rpn.proposal_layer.n_test_post_nms = n_rpn
        return rois
